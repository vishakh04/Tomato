...
const Cart = () => {
  ...
  const applyPromoCode = () => {
    if (promoCode.toLowerCase() === 'new50') {
      setDiscount(getTotalCartAmount() * 0.5); // Apply 50% discount
      setErrorMessage('');
    } else {
      setErrorMessage('Invalid promo code'); // Handle invalid promo code
      setDiscount(0);
    }
  };

  const totalAmount = getTotalCartAmount() - discount + (getTotalCartAmount() === 0 ? 0 : 5); // Subtract discount and add delivery fee
